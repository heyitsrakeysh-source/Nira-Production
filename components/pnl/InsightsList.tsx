      ...
      {insights.map((i, idx) => {
        ...
        return (
          <li key={i.id} style={{ ["--i" as string]: idx }}>
            <Link
              href={href}
              className="group flex gap-3 rounded-lg border border-line bg-surface-2 p-3 transition-[background-color,border-color,transform] duration-200 can-hover:-translate-y-px hover:border-line-strong hover:bg-surface"
            >
              <span
                className={cn(
              ...
              <ArrowRight
                size={14}
                className="mt-1 shrink-0 -translate-x-1 text-ink-4 opacity-0 transition-[transform,opacity] duration-[var(--dur)] ease-[var(--ease-out)] group-can-hover:translate-x-0 group-hover:opacity-100"
              />
            </Link>
          </li>
        );
