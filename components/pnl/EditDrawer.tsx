...
export function EditDrawer({
  ...
  // Only interrupt when there is something to lose. Confirming a no-op close
  // would be the kind of dialog people learn to dismiss without reading.
  const requestClose = () => {
    if (changeCount > 0) setConfirmDiscard(true);
    else onClose();
  };
    ...
    <Drawer
      ...
      overlay={
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-[var(--overlay)] p-6">
          <div className="anim-scale-in w-full max-w-sm rounded-lg border border-line bg-surface p-5 shadow-pop">
            <p className="text-[15px] font-semibold text-ink">Discard {changeCount} unsaved change{changeCount === 1 ? "" : "s"}?</p>
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-ink-3">
              {month.label} will go back to its synced values. This cannot be undone.
            </p>
          ...
          <div className="grid grid-cols-3 divide-x divide-line border-b border-line bg-surface-2">
            ...
            >
              {Math.abs(preview.netProfit - month.netProfit) > 0.5 ? (
                ...
                <span className="text-[11.5px] text-ink-4">{money(preview.netProfit)} net</span>
              )}
            </LiveStat>
          </div>
            ...
            {EDITOR_SCHEMA.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={cn(
                      "tnum rounded-full px-1.5 py-px text-[11.5px] font-bold",
                ...
                <section key={section.id} className="overflow-hidden rounded-lg border border-line">
                  <header className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-line bg-surface-2 px-4 py-2.5">
                    ...
                    <span className="ml-auto flex items-center gap-2">
                      <span className="tnum text-[12.5px] font-semibold text-ink-2">{money(sectionTotal)}</span>
                      <span
                        className={cn(
                          "tnum rounded-full px-1.5 py-0.5 text-[11.5px] font-bold",
                  ...
                  <div className="divide-y divide-line-soft">
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-1.5 border-t border-dashed border-line py-2.5 text-[12.5px] font-medium text-ink-3 transition-colors hover:bg-surface-2 hover:text-brand-ink"
                    >
                      <Plus size={13} />
                      Add another field
                    </button>
    ...
    >
      ...
      <div className="min-w-[190px] flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <span
            className="rounded bg-surface-3 px-1.5 py-px text-[11.5px] font-semibold tracking-wide text-ink-4 "
            title={source.label}
          >
            {source.short}
          </span>
      ...
      <div className="relative">
        <span className="pointer-events-none absolute inset-y-0 left-2.5 flex items-center text-[12.5px] text-ink-4">
          {unit === "inr" ? "₹" : ""}
        </span>
        <input
          inputMode="decimal"
        ...
        />
        {unit === "pct" ? (
          <span className="pointer-events-none absolute inset-y-0 right-2.5 flex items-center text-[12.5px] text-ink-4">
            %
          </span>
        ) : null}
      </div>
      ...
      {!field.readOnly && lastMonth !== null ? (
        <button
          ...
          className="rounded-md border border-line px-2 py-1 text-[11.5px] font-medium text-ink-3 opacity-0 transition-[opacity,color,border-color] group-hover:opacity-100 hover:border-brand hover:text-brand-ink focus-visible:opacity-100"
        >
          Use last month
        </button>
      ) : null}
      ...
      {field.recurring ? (
        <span className="flex items-center gap-1.5">
          <span className="text-[11.5px] text-ink-4">Recurs</span>
        ...
        {menu ? (
          <>
            ...
            <div className="anim-scale-in absolute top-full right-0 z-20 mt-1 w-52 origin-top-right rounded-lg border border-line bg-surface p-1.5 shadow-pop">
              <p className="px-2 py-1 text-[11.5px] text-ink-4">{source.label}</p>
              <Divider className="my-1" />
              {["Split across months", "Mark as estimate", "Add a comment", "View change history"].map((a) => (
                <button
                  key={a}
                  onClick={() => setMenu(false)}
                  className="w-full rounded-md px-2 py-1.5 text-left text-[12.5px] text-ink-2 transition-colors hover:bg-surface-3 hover:text-ink"
                >
                  {a}
                </button>
              ))}
