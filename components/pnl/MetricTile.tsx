...
export function MetricTile({
  ...
  return (
    <div
      className="group relative flex flex-col rounded-lg border border-line bg-surface p-4 shadow-xs transition-[box-shadow,border-color,transform] duration-200 can-hover:-translate-y-px hover:border-line-strong hover:shadow-md"
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
        <AnimatedNumber
          ...
          className={cn(
            "figure-lg text-[26px] leading-none sm:text-[26px]",
  ...
  return (
    <div ref={ref} className="relative min-w-0 flex-1">
      <button
        ...
        aria-expanded={open}
        className="-ml-1 flex max-w-full items-center gap-1 rounded px-1 py-0.5 text-[12.5px] font-medium text-ink-3 transition-colors hover:bg-surface-3 hover:text-ink"
      >
        <span className="truncate">{def.term ? termLabel(def.term, plainMode) : def.label}</span>
        <ChevronDown
          size={12}
          className={cn(
        ...
        >
          {groups.map((g) => (
            <div key={g} className="mb-1">
              <p className="label-xs px-2 py-1 text-[11.5px]">{g}</p>
              {METRICS.filter((m) => m.group === g).map((m: MetricDef) => {
                ...
                return (
                  ...
                  >
                    <span className="truncate">{m.term ? termLabel(m.term, plainMode) : m.label}</span>
                    {selected ? <Check size={13} className="shrink-0" /> : null}
                    {used ? <span className="shrink-0 text-[11.5px]">shown</span> : null}
                  </button>
                );
              })}
            </div>
...
export function useTileChoice(defaults: string[]) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("nira-tiles");
      if (raw) {
  ...
  const setAt = (index: number, id: string) => {
    setIds((prev) => {
      ...
      next[index] = id;
      try {
        localStorage.setItem("nira-tiles", JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
