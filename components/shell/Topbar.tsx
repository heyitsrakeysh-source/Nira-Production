...
export function Topbar({
  ...
  const { brand, brandId, setBrandId, months, plainMode, setPlainMode } = useWorkspace();
  ...
  // Scroll edge effect: no divider until there is content behind the bar.
  useEffect(() => {
  ...
  return (
    <header
      data-print-hide
      className={cn(
        "sticky top-0 z-40 flex h-[60px] items-center gap-3 px-4 sm:px-6",
        "bg-surface",
        "border-b transition-[border-color,box-shadow] duration-[var(--dur)] ease-[var(--ease-out)]",
    ...
    >
      ...
      <div ref={wrapRef} className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            <span className="grid size-6 shrink-0 place-items-center rounded-[6px] bg-brand text-[11.5px] font-bold text-white">
              {brand.initials}
            </span>
            <span className="hidden min-w-0 sm:block">
              <span className="block truncate text-[13.5px] leading-tight font-semibold text-ink">{brand.name}</span>
              <span className="block truncate text-[11.5px] leading-tight text-ink-4">{brand.category}</span>
            </span>
            ...
            <div className="anim-scale-in absolute top-full left-0 z-50 mt-1.5 w-64 origin-top-left rounded-lg border border-line bg-surface p-1.5 shadow-pop">
              ...
              {BRAND_PROFILES.map((b) => {
                ...
                return (
                  ...
                  >
                    <span className="grid size-7 shrink-0 place-items-center rounded-md bg-brand text-[11.5px] font-bold text-white">
                      {b.initials}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[13.5px] font-medium text-ink">{b.name}</span>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button
            onClick={() => {
              ...
              push({
                title: plainMode ? "Showing finance terms" : "Showing plain language",
                body: plainMode
                  ? "Labels now read CAC, AOV, contribution and so on."
                  : "Labels now read in everyday words. Hover any of them for the finance term.",
                tone: "info",
              });
            }}
            title={plainMode ? "Switch to finance terms" : "Switch to plain language"}
            className={cn(
              "hidden h-9 items-center gap-1.5 rounded-md border px-2.5 text-[12.5px] font-medium transition-colors lg:flex",
              plainMode
          ...
          >
            <Languages size={14} />
            {plainMode ? "Plain English" : "Finance terms"}
          </button>
          ...
          <div className="relative">
            ...
            >
              <Bell size={16} />
              {unread > 0 ? (
                <span className="absolute -top-0.5 -right-0.5 grid size-[15px] place-items-center rounded-full bg-critical text-[11.5px] font-bold text-white ring-2 ring-surface">
                  {unread}
                </span>
              ) : null}
            </IconButton>
                      ...
                      <li key={n.id}>
                        <div className="flex gap-2.5 rounded-md p-2.5 transition-colors hover:bg-surface-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 text-[11.5px] text-ink-4">{n.time}</p>
                          </div>
                        </div>
                      </li>
                    );
          ...
          <div className="relative">
            ...
            >
              <span className="grid size-7 shrink-0 place-items-center rounded-full bg-brand text-[11.5px] font-bold text-[#173f28]">
                {CURRENT_USER.initials}
              </span>
              <span className="hidden text-[13.5px] font-medium text-ink lg:block">{CURRENT_USER.name}</span>
            ...
            {menu === "user" ? (
              <div className="anim-scale-in absolute top-full right-0 z-50 mt-1.5 w-60 origin-top-right rounded-lg border border-line bg-surface p-1.5 shadow-pop">
                <div className="border-b border-line px-2.5 pb-2.5 pt-1.5">
                  <p className="text-[13.5px] font-semibold text-ink">{CURRENT_USER.name}</p>
                ...
                <div className="pt-1.5">
                  {[
                    ...
                    <button
                      ...
                      className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-[13.5px] text-ink-2 transition-colors hover:bg-surface-3 hover:text-ink"
                    >
