...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-[10vh]" role="dialog" aria-modal="true" aria-label="Command palette">
      <button aria-label="Close" onClick={onClose} className="anim-fade-in absolute inset-0 cursor-default" style={{ background: "var(--overlay)" }} />
      ...
      <div className="anim-scale-in relative w-full max-w-[620px] overflow-hidden rounded-xl border border-line bg-surface shadow-lg">
        <div className="flex items-center gap-2.5 border-b border-line px-4">
          ...
          <input
            ...
            className="h-13 w-full bg-transparent py-4 text-[13.5px] text-ink placeholder:text-ink-4 focus:outline-none"
          />
          <Kbd>esc</Kbd>
        </div>
        ...
        <ul ref={listRef} className="max-h-[52vh] overflow-y-auto p-2">
          {grouped.length === 0 ? (
            <li className="px-3 py-10 text-center text-[13.5px] text-ink-3">
              No matches for “{query}”
            </li>
          ) : null}
          {grouped.map(([group, items]) => (
            <li key={group} className="mb-1">
              <p className="label-xs px-2.5 py-1.5 text-[11.5px]">{group}</p>
              <ul>
                {items.map((c) => {
                  index += 1;
        ...
        <div className="flex items-center gap-4 border-t border-line bg-surface-2 px-4 py-2.5 text-[11.5px] text-ink-4">
