            ...
            return (
              <li key={o.value}>
                <button
                  ...
                  className={cn(
                    "flex w-full items-center justify-between gap-6 rounded-[6px] px-2.5 py-1.5 text-left text-[13.5px] transition-colors",
                    selected ? "bg-brand-soft text-brand-ink" : "text-ink-2 hover:bg-surface-3 hover:text-ink",
                  )}
                >
                  <span className="flex flex-col">
                    <span className="font-medium">{o.label}</span>
                    {o.hint ? <span className="text-[11.5px] text-ink-4">{o.hint}</span> : null}
                  </span>
