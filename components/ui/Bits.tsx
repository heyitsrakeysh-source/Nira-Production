...
export function ProgressRing({
  ...
  return (
    <div className={cn("relative shrink-0", className)} style={{ width: size, height: size }}>
      ...
      {children ? (
        <div className="absolute inset-0 grid place-items-center text-[11.5px] font-bold tabular-nums text-ink">
          {children}
        </div>
      ) : null}
    </div>
...
export function Kbd({ children }: { children: ReactNode }) {
  return (
    <kbd className="inline-flex h-[18px] min-w-[18px] items-center justify-center rounded border border-line bg-surface-2 px-1 font-sans text-[11.5px] font-semibold text-ink-3">
      {children}
    </kbd>
  );
}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-6 py-12 text-center">
      <div className="grid size-10 place-items-center rounded-full bg-surface-3 text-ink-3">{icon}</div>
      <div>
        <p className="text-[13.5px] font-semibold text-ink">{title}</p>
        <p className="mx-auto mt-1 max-w-sm text-[12.5px] leading-relaxed text-ink-3">{body}</p>
      </div>
      {action}
    </div>
  );
