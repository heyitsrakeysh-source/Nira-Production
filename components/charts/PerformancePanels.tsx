...
const STACK = [
  { id: "cogs", term: "cogs", color: "var(--series-1)", get: (m: MonthFigures) => m.cogs },
  { id: "ops", term: "opCosts", color: "var(--series-3)", get: (m: MonthFigures) => m.totalOperationalCosts },
  { id: "marketing", term: "marketing", color: "var(--series-5)", get: (m: MonthFigures) => m.totalMarketing },
];

export function PerformancePanels({ months, height = 292 }: { months: MonthFigures[]; height?: number }) {
  const { plainMode } = useWorkspace();
  const stack = STACK.map((s) => ({ ...s, label: termLabel(s.term, plainMode, true) }));
  ...
  const legend: LegendItem[] = [
    ...
    { label: termLabel("revenue", plainMode, true), color: "var(--ink-2)", shape: "line" as const },
    { label: termLabel("netProfit", plainMode, true), color: "var(--brand)", shape: "square" as const },
  ];
      ...
      <div ref={ref} className="relative w-full" style={{ height }}>
        {width > 0 ? (
          <svg width={width} height={height} role="img" aria-label="Revenue, cost composition and net profit by month">
            ...
            <text x={M.left} y={M.top - 10} fontSize={11} fontWeight={700} fill="var(--ink-4)">
              Revenue &amp; costs
            </text>
            <GridLines
              ticks={topAxis.ticks}
            ...
            {/* ---------- panel 2: net profit ---------- */}
            <text x={M.left} y={botTop - 10} fontSize={11} fontWeight={700} fill="var(--ink-4)">
              Net profit
            </text>
            <GridLines
              ticks={botAxis.ticks}
