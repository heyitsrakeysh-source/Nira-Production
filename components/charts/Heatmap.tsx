  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] border-separate border-spacing-0 text-[12.5px]">
        ...
        <thead>
          <tr>
            <th scope="col" className="sticky left-0 z-10 bg-surface pb-2 pr-3 text-left text-[11.5px] font-semibold tracking-wide text-ink-3 ">
              SKU
            </th>
            {COLUMNS.map((c) => (
              <th key={c.id} scope="col" className="pb-2 pl-1.5 text-right text-[11.5px] font-semibold tracking-wide text-ink-3 ">
                {c.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((s, ri) => (
            <tr key={s.id} className="group">
              <th
                scope="row"
                className="sticky left-0 z-10 max-w-[180px] truncate bg-surface py-[3px] pr-3 text-left text-[12.5px] font-medium text-ink transition-colors group-hover:bg-surface-2"
              >
                  ...
                  <td key={c.id} className="py-[3px] pl-1.5">
                    <div
                      className={cn(
                        ...
                        "transition-transform duration-150 group-can-hover:scale-[1.015]",
                      )}
