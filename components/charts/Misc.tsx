    ...
    <div className="py-2">
      <div className="mb-1.5 flex items-baseline justify-between gap-3">
        ...
        <span className="flex items-baseline gap-2">
          <span className={cn("tnum text-[13.5px] font-bold", winning ? "text-good-ink" : "text-critical-ink")}>
            {format(you)}
          </span>
  ...
  return (
    <div className="flex flex-col items-center" style={{ width: size }}>
      ...
      <div className="-mt-5 text-center">
        ...
        {label ? <p className="mt-0.5 text-[11.5px] text-ink-4">{label}</p> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ left: `${pos}%` }}>
      ...
      >
        ...
        <span className="block text-[11.5px] text-ink-4">{label}</span>
      </span>
    </div>
  );
}
