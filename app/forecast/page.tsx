    ...
    <PageShell>
      ...
      <Card className="mt-4">
        <CardHeader
          ...
          action={
            <Link href="/explorer" className="text-[12.5px] font-medium text-brand-ink hover:underline">
        ...
        <ul className="stagger mt-4 grid grid-cols-1 gap-2.5 md:grid-cols-2 xl:grid-cols-3">
          {levers.map((l, i) => (
            <li
              key={l.id}
              style={{ ["--i" as string]: i }}
              className="group rounded-lg border border-line bg-surface-2 p-3.5 transition-[border-color,transform,box-shadow] duration-200 can-hover:-translate-y-px hover:border-line-strong hover:shadow-sm"
            >
            ...
            {losers.slice(0, 6).map((s, i) => (
              <li key={s.id} className="flex items-center gap-3 py-2.5">
                ...
                <span className="shrink-0 text-right">
                  ...
                  <span className="tnum block text-[11.5px] text-ink-4">{money(s.contributionPerOrder)}/order</span>
                </span>
              </li>
            ))}
          </ul>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <span className="text-[11.5px] text-ink-4">{a.time}</span>
                      <Link
                        href={a.action.href}
                        className="rounded-md border border-line bg-surface px-2 py-1 text-[11.5px] font-medium text-brand-ink transition-colors hover:border-brand"
                      >
                        {a.action.label}
                      </Link>
                    </div>
