...
export default function GuidePage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("nira-guide-progress");
  ...
  const toggle = (id: string) => {
    setDone((prev) => {
      ...
      try {
        localStorage.setItem("nira-guide-progress", JSON.stringify([...next]));
      } catch {
        /* ignore */
      }
      return next;
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-[228px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <Card className="aurora overflow-hidden">
            ...
            <h2 className="text-[15px] font-semibold text-ink">What you are looking at</h2>
                  ...
                  return (
                    <div key={stage.id} className="overflow-hidden rounded-lg border border-line">
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-line bg-surface-2 px-4 py-3">
                        ...
                        <span
                          className={cn(
                            "tnum rounded-full px-2 py-0.5 text-[11.5px] font-bold",
                          ...
                          return (
                            <li key={t.id}>
                              ...
                              >
                                ...
                                <span className="shrink-0 rounded-md bg-surface-3 px-1.5 py-0.5 text-[11.5px] font-bold text-ink-3">
                                  {t.effort}
                                </span>
                              </button>
                            </li>
            ...
            <Card>
                  ...
                  <thead className="bg-surface-2">
                    <tr>
                      {["Area", "File", "What is fake", "What to do"].map((h) => (
                        <th key={h} scope="col" className="border-b border-line px-3 py-2.5 text-left text-[11.5px] font-semibold tracking-wide text-ink-3 ">
                          {h}
                        </th>
                      ))}
                    </tr>
              ...
              <div className="mt-4 rounded-lg border border-line bg-brand-soft p-4">
                ...
                <p className="mt-1.5 text-[11.5px] leading-relaxed text-brand-ink/90">
                  <code className="font-[family-name:var(--font-mono)]">computeMonth()</code> in <FilePath>lib/data/model.ts</FilePath> is the
                    ...
                    {API_ROUTES.map((r) => (
                      <tr key={r.method + r.path} className="transition-colors hover:bg-surface-2">
                        <td className="border-b border-line-soft px-3 py-2.5">
                          <span
                            className={cn(
                              "rounded px-1.5 py-0.5 font-[family-name:var(--font-mono)] text-[11.5px] font-bold",
                        ...
                        <td className="border-b border-line-soft px-3 py-2.5 font-[family-name:var(--font-mono)] text-[11.5px] whitespace-nowrap text-ink">{r.path}</td>
                        <td className="border-b border-line-soft px-3 py-2.5 text-ink-2">{r.purpose}</td>
                        <td className="border-b border-line-soft px-3 py-2.5 font-[family-name:var(--font-mono)] text-[11.5px] whitespace-nowrap text-ink-3">{r.returns}</td>
                      ...
                      {vars.map((v, i) => (
                        <li key={v.key} className={cn("flex flex-wrap items-start gap-x-4 gap-y-1 px-4 py-3", i > 0 && "border-t border-line-soft")}>
                          <div className="min-w-[230px]">
                            <code className="font-[family-name:var(--font-mono)] text-[12.5px] font-semibold text-ink">{v.key}</code>
                            {v.required ? (
                              <span className="ml-2 rounded bg-critical-soft px-1.5 py-px text-[11.5px] font-bold tracking-wide text-critical-ink ">
                                required
                              </span>
                            ) : (
                              <span className="ml-2 rounded bg-surface-3 px-1.5 py-px text-[11.5px] font-bold tracking-wide text-ink-4 ">
                                optional
                              </span>
                            )}
                          </div>
                          <p className="min-w-[200px] flex-1 text-[11.5px] text-ink-2">{v.purpose}</p>
                          <code className="max-w-full truncate font-[family-name:var(--font-mono)] text-[11.5px] text-ink-4">{v.example}</code>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2">
                <ol className="space-y-3">
                  <Step n={1} title="Install a provider">
                    ...
                    only needs a session with a <code className="font-[family-name:var(--font-mono)]">workspaceId</code>.
                  </Step>
                  ...
                  <Step n={3} title="Feed the topbar">
                    <FilePath>components/shell/Topbar.tsx</FilePath> reads <code className="font-[family-name:var(--font-mono)]">CURRENT_USER</code>{" "}
                    and <code className="font-[family-name:var(--font-mono)]">BRANDS</code> from the demo file. Replace with the session user and
                    their memberships.
                  </Step>
            ...
            <Card>
                  ...
                  <tbody>
                    {DB_TABLES.map((t) => (
                      <tr key={t.name} className="transition-colors hover:bg-surface-2">
                        <td className="border-b border-line-soft px-3 py-2.5 font-[family-name:var(--font-mono)] text-[11.5px] font-semibold whitespace-nowrap text-brand-ink">
                          {t.name}
                        </td>
                        <td className="border-b border-line-soft px-3 py-2.5 text-ink-2">{t.purpose}</td>
                        <td className="border-b border-line-soft px-3 py-2.5 font-[family-name:var(--font-mono)] text-[11.5px] text-ink-3">{t.columns}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-line bg-surface-2 p-3.5">
                ...
                <p className="text-[11.5px] leading-relaxed text-ink-3">
                  ...
                  <code className="font-[family-name:var(--font-mono)]">DATABASE_URL</code>. Use a pooled connection for the app and{" "}
                  <code className="font-[family-name:var(--font-mono)]">DIRECT_URL</code> for migrations. Compute monthly drivers in a scheduled
                ...
                {[
                  ...
                  <li key={i.name} className="rounded-lg border border-line bg-surface-2 p-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="ml-auto rounded bg-surface-3 px-1.5 py-0.5 font-[family-name:var(--font-mono)] text-[11.5px] text-ink-4">{i.scopes}</span>
                    </div>
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <ol className="space-y-3">
                  <Step n={1} title="Open the folder in GitHub Desktop">
                    File → Add Local Repository → choose <FilePath>nira-profit-os</FilePath>. A git repository is
                  ...
                  <Step n={2} title="Publish the repository">
                    ...
                    <code className="font-[family-name:var(--font-mono)]">node_modules</code>, <code className="font-[family-name:var(--font-mono)]">.next</code> and{" "}
                    <code className="font-[family-name:var(--font-mono)]">.env*.local</code>.
                  </Step>
                  ...
                  <Step n={5} title="Deploy">
                    Every push to <code className="font-[family-name:var(--font-mono)]">main</code> ships to production; every branch gets a
...
git remote add origin https://github.com/<you>/nira-profit-os.git
git branch -M main
...
function Step({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <li className="flex gap-3 rounded-lg border border-line bg-surface-2 p-3.5">
      <span className="grid size-6 shrink-0 place-items-center rounded-full bg-brand text-[11.5px] font-bold text-white">
        {n}
      </span>
