...
export default function ExplorerPage() {
  const { current, comparison, completeness, channel, setChannel, plainMode } = useWorkspace();
  const t = (id: string, short = false) => termLabel(id, plainMode, short);
  const { push } = useToast();
  ...
  return (
    <PageShell>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 rounded-lg border border-line bg-surface p-4 shadow-xs md:grid-cols-3 xl:grid-cols-5">
        {[
          { k: t("revenue"), v: money(current.totalRevenue), cur: current.totalRevenue, prev: comparison.totalRevenue, better: true },
          { k: t("grossMargin"), v: pct(current.grossMarginPct), cur: current.grossMarginPct, prev: comparison.grossMarginPct, better: true, pp: true },
          { k: t("cac", true), v: money(current.cac), cur: current.cac, prev: comparison.cac, better: false },
          { k: t("contribution"), v: `${money(current.contributionPerOrder)} /order`, cur: current.contributionPerOrder, prev: comparison.contributionPerOrder, better: true },
          { k: t("netProfit"), v: money(current.netProfit), cur: current.netProfit, prev: comparison.netProfit, better: true },
        ].map((s) => (
          <div key={s.k} className="min-w-0">
            <p className="text-[11.5px] font-medium text-ink-3">{s.k}</p>
            <div className="mt-1 flex flex-wrap items-baseline gap-2">
              <span className={cn("figure-lg text-[17px]", s.k === "Net profit" && s.cur < 0 ? "text-critical-ink" : "text-ink")}>
                {s.v}
              </span>
                  ...
                  <li key={v.id}>
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        <span className="block truncate text-[12.5px] font-medium">{v.name}</span>
                        <span className="block text-[11.5px] text-ink-4">{v.description}</span>
                      </span>
                    </button>
                  </li>
                ))}
