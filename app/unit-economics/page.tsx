  ...
  return (
    <PageShell>
      ...
      <Card className="mt-6">
        ...
        <ContributionExplainer month={current} className="mt-3" />
        <div className="mt-5 grid grid-cols-2 gap-3 border-t border-line pt-4 sm:grid-cols-4">
          {[
            ...
            <div key={s.k}>
              <p className="text-[11.5px] font-medium text-ink-4">{s.k}</p>
              <p
                className={cn(
      ...
      <div className="stagger mt-3 grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
        {tiles.map((t, i) => (
          <div
            key={t.label}
            style={{ ["--i" as string]: i }}
            className="rounded-lg border border-line bg-surface p-3.5 shadow-xs transition-[box-shadow,transform] duration-200 can-hover:-translate-y-px hover:shadow-md"
          >
      ...
      <Card className="mt-3">
        ...
        {rows.length === 0 ? (
          <EmptyState
            ...
            body={
              query.trim()
                ? `Nothing in ${category === "All categories" ? "the catalogue" : category} matches "${query.trim()}".`
                : `No products in ${category} this month.`
            }
            action={
              <Button
                size="sm"
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-line">
          <table className="w-full min-w-[900px] border-collapse text-[12.5px]">
            ...
            <thead className="bg-surface-2">
              <tr>
                <th scope="col" className="sticky left-0 z-10 border-b border-line bg-surface-2 px-3 py-2.5 text-left text-[11.5px] font-semibold tracking-wide text-ink-3 ">
                  SKU
                </th>
                {(
                  [
                  ...
                  <th key={key} scope="col" className="border-b border-line px-3 py-2.5 text-right">
                    <button
                      ...
                      className={cn(
                        "inline-flex items-center gap-1 text-[11.5px] font-semibold tracking-wide transition-colors",
