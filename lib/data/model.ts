...
export const BRAND_PROFILES: BrandProfile[] = [
  ...
  },
  {
    ...
    plan: "Enterprise",
    category: "Sportswear",
    orders: 3.1, aov: 1.18, cogs: 0.86, cac: 1.16, fixed: 2.2, shipping: 0.72, returns: 0.82,
    cacDrift: 0.9,
    headline: "High-volume sportswear with strong demand and disciplined margin control.",
  },
];
...
/**
 * Seating order for the pie.
 *
 * A ring wraps, so the last slice sits next to the first. In definition order
 * that puts indigo beside violet, a pair colourblind readers cannot separate.
 * This order keeps each category's own colour and only changes where it sits,
 * and it clears the CVD gate in both themes including the wrap.
 */
export const RING_SLOT_ORDER = [1, 2, 3, 6, 5, 4];

export function costBreakdownRing(m: MonthFigures) {
  const { parts } = costBreakdown(m);
  return RING_SLOT_ORDER.map((slot) => parts.find((p) => p.slot === slot)!).filter(Boolean);
}
