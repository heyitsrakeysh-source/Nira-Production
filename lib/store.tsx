...
interface WorkspaceValue {
  ...
  /** Swaps finance vocabulary for plain language across the whole product. */
  plainMode: boolean;
  ...
  /** Single-cell commit, used by the sheet's autosave. */
  setFieldValue: (monthKey: MonthKey, fieldId: string, value: number | null) => void;
  /** True when an integration-fed cell carries a manual value instead. */
  isOverridden: (monthKey: MonthKey, fieldId: string) => boolean;
  /** Drop a manual value so the cell goes back to its synced figure. */
  revertField: (monthKey: MonthKey, fieldId: string) => void;
...
/** Whether a bound field's driver has actually been written in this override. */
function hasBoundOverride(o: DriverOverride, field: EditorField) {
  if (!field.bind) return false;
  if (field.bind.kind === "fixed") return o.fixed?.[field.bind.part] !== undefined;
  return (o as Record<string, unknown>)[field.bind.driver as string] !== undefined;
}
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const [plainMode, setPlainModeRaw] = useState(true);
  ...
  const setFieldValue = useCallback(
    (monthKey: MonthKey, fieldId: string, value: number | null) => {
      commitEdits(monthKey, { [fieldId]: value });
  ...
  const isOverridden = useCallback(
    (monthKey: MonthKey, fieldId: string) => {
      const field = ALL_FIELDS.find((f) => f.id === fieldId);
      if (!field) return false;
      if (field.bind) return overrides[monthKey] !== undefined && hasBoundOverride(overrides[monthKey], field);
      return manualValues[monthKey]?.[fieldId] !== undefined;
    },
    ...
    (monthKey: MonthKey, fieldId: string) => {
      startTransition(() => {
        ...
        setManualByBrand((prev) => {
          ...
          const forMonth = { ...(forBrand[monthKey] ?? {}) };
          delete forMonth[fieldId];
          forBrand[monthKey] = forMonth;
          return { ...prev, [brandId]: forBrand };
        });
        if (field?.bind) {
          setOverridesByBrand((prev) => {
            const forBrand = { ...(prev[brandId] ?? {}) };
            const o = { ...(forBrand[monthKey] ?? {}) };
            if (field.bind!.kind === "fixed") {
              const fixed = { ...(o.fixed ?? {}) };
              delete fixed[field.bind!.part];
              o.fixed = Object.keys(fixed).length ? fixed : undefined;
            } else {
              delete (o as Record<string, unknown>)[field.bind!.driver as string];
            }
            forBrand[monthKey] = o;
            return { ...prev, [brandId]: forBrand };
          });
  ...
  const setPlainMode = useCallback((v: boolean) => {
    setPlainModeRaw(v);
    try {
      localStorage.setItem("nira-plain", v ? "1" : "0");
    } catch {
      /* preference only */
    }
  }, []);
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("nira-plain");
      if (raw !== null) setPlainModeRaw(raw === "1");
    } catch {
      /* preference only */
    }
  }, []);
